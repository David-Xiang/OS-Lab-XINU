...
/*------------------------------------------------------------------------
 * xsh_udpechot - shell command that can send a message to a remote UDP
 *			echo server and receive a reply **using timer**
 *------------------------------------------------------------------------
 */
shellcmd xsh_udpechot(int nargs, char *args[])
{
	...
	*/
	...
	for (i=0; i<retries; i++) {
		...
		retval = udp_recvt(slot, inbuf, sizeof(inbuf), delay);
		if (retval == TIMEOUT) {
