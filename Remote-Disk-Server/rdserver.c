...
// /*DEBUG*/ printf("DEBUG: message length is %d  and minimum is %d\n",
// /*DEBUG*/			n, sizeof(struct rd_msg_hdr));
...
/*DEBUG*/ printf("DEBUG: ignoring because sequence is bad \n");
		...
		if (len >= RD_IDLEN) {
/*DEBUG*/ printf("DEBUG: ignoring ID that's too long\n");
			snderr(mptr, rptr, sizeof(struct rd_msg_hdr) );
			continue;
		}
// /*DEBUG*/ printf("DEBUG: ID is %s\n", mptr->rd_id);
		...
		if (err > 0) {
			snderr(mptr, rptr, sizeof(struct rd_msg_hdr) );
/*DEBUG*/ printf("DEBUG: ignoring because ID contains /\n");
			continue;
		}
